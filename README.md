[[Home](README.md) | [Quick Start](docs/quick-start.md) | [Overview](docs/overview.md) | [Features](docs/features.md) | [Installation](docs/installation/README.md) | [Settings](docs/settings/README.md) | [Integrations](docs/integrations/README.md) | [Troubleshooting](docs/troubleshooting.md) | [FAQ](docs/faq.md)

---

# 🌌 Nuvio Wiki - Everything you wanted to know about Nuvio, and some stuff you don't

Welcome to the unofficial hub for **Nuvio**, a modern media player and aggregator. This repository provides comprehensive guides and troubleshooting for users on all platforms.

> [!IMPORTANT]  
> **Disclaimer:** This is **NOT** an official Nuvio wiki. This project is community-maintained and is not affiliated with the official Nuvio development team.

> [!NOTE]
> Nuvio is free and open source. [Please consider donating to keep free and open source projects running](https://tapframe.space/contribute). FOSS software plays a vital role in the world and it is up to the users to keep these projects running.

If you need something to help you get through this, here is a [1-hour Subway Surfers video for your enjoyment](https://www.youtube.com/watch?v=ChBg4aowzX8). 🚆🏃‍♂️

## 📚 Documentation

### 🚀 Getting Started
*   [**Quick Start Guide**](docs/quick-start.md) - Get up and running.
*   [General Overview](docs/overview.md) - How Nuvio works.
*   [Glossary](docs/glossary.md) - Common terms explained.
*   [Official Links](docs/official-links.md) - Important Nuvio links.

### 📥 Installation
*   [Installation Guide Overview](docs/installation/README.md) - Readme for installation.
*   [Android TV](docs/installation/android-tv.md) - Play Store or APK.
*   [Android Mobile](docs/installation/android-mobile.md) - Play Store or APK.
*   [iOS / Sideloading](docs/installation/ios.md) - Installation via Sideloading or TestFlight.
*   [WebOS](docs/installation/webos.md) - Experimental Support.

### ⚙️ Configuration & Integrations
*   [Addons Guide](docs/addons/README.md) - Content, Metadata, and Utility addons.
*   [Settings Breakdown](docs/settings/README.md) - General UI and settings info.
*   [Player Settings](docs/settings/player.md) - Playback configuration.
*   [Debrid Integration](docs/integrations/debrid.md) - Link your Debrid services.
*   [IMDb, MDBList, Trakt](docs/integrations/imdb-mdblist-trakt.md) - Account integrations.
*   [Custom Collections](docs/settings/collections.md) - Create deep custom collections.
*   [Profile Management](docs/settings/profiles.md) - Manage multiple users.

### 🆘 Support & Info
*   [Troubleshooting](docs/troubleshooting.md) - Fix common issues.
*   [FAQ](docs/faq.md) - Frequently asked questions.
*   [Feature List](docs/features.md) - What Nuvio can do.

---

If you would like to support some of the developers of Nuvio:

*   [skoruppa](https://ko-fi.com/skoruppa)
*   [halibiram](https://ko-fi.com/halibiram)
*   [𝔈𝔡𝔬](https://ko-fi.com/edoedac)
*   [Hond](https://torbox.app/subscription?referral=41d1ac85-ee5e-4699-9f0a-92e67cbc2fb241d1ac85-ee5e-4699-9f0a-92e67cbc2fb2) - Torbox Referral

---

### 🤝 Contribution
Contributions are vital! Please see the [Contributing Guide](CONTRIBUTING.md) to help improve these docs.
